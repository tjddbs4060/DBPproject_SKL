...
//일정 갯수를 하나하나 확인
	...
	$query = "select s.* from schedule as s, entry as e where s.sch_index = e.sch_index and e.id = '".$_SESSION['userid']."' and s.sch_date = '".$_GET['Y']."-".$_GET['M']."-".$i."'";
	$result = mysql_query($query);
...
//일정 출력
	...
	$query = "select s.*, e.join_index from schedule as s, entry as e where s.sch_index = e.sch_index and e.id = '".$_SESSION['userid']."' and s.sch_date = '$date_now'";
	$result = mysql_query($query);

	if (mysql_fetch_assoc($result)) {
		$result = mysql_query($query);
		$div_string .= '<tr> <td colspan = "3"> <div class = "sch" style = "background-color : #ffffff; width : 300px; border : 3px solid #5ac7db; padding-right : 20px; padding-left : 20px; word-wrap : break-word; line-height : 25px;">';
		$i = 0;

		$div_string .= "<h4 style = 'text-align : center; font-family : serif;'> <b> <참여하는 일정> </b> </h4>";
		while ($arr_list = mysql_fetch_assoc($result)) {
			if (!$i) $div_string .= "<table class = 'sch' style = 'width : 300px;'>";
			if ($arr_list['friend_range']) $div_string .= "<tr> <td onclick = 'window.location.replace(\"check_sch.php?index=".$arr_list['sch_index']."\");' style = 'max-width : 270px; word-wrap : break-word;'> <font class = 'small'>".$arr_list['sch_start_time']." ~ ".$arr_list['sch_finish_time']." / ";
			else $div_string .= "<tr> <td style = 'max-width : 270px; word-wrap : break-word;'> <font class = 'small'>".$arr_list['sch_start_time']." ~ ".$arr_list['sch_finish_time']." / ";
			...
			$div_string .= "</font><br>".$arr_list['content']."</td> <td style = 'text-align : right; width : 30px;'>";
			$div_string .= "<img src = 'img/delete(default).png' onclick = 'window.location.replace(\"del_ent.php?value=".$_GET['value']."&index=".$arr_list['join_index']."&Y=".$_GET['Y']."&M=".$_GET['M']."\");' onmouseover = 'this.src = \"img/delete(over).png\";' onmouseout = 'this.src = \"img/delete(default).png\";'>";
			$div_string .= "</td> <tr style = 'height : 10px'> </tr> </tr>";
			$i = 1;
		}
		if ($i) $div_string .= "</table>";
